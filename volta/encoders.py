...
class BertGatedAttention(nn.Module):
    ...
    def forward(self, t_input_tensor, v_input_tensor, t_attention_mask, v_attention_mask):
        ...
        #t_attn_output:把attention score扔去ln，t_attn_probs 是none 如果没有可视化
...
class BertTextPooler(nn.Module):
    def __init__(self, config):
        ...
        self.dense = nn.Linear(config.hidden_size, config.pooler_size) #[768,1024]
...
class BertImagePooler(nn.Module):
    def __init__(self, config):
        ...
        self.dense = nn.Linear(config.v_hidden_size, config.v_pooler_size) #[768,1024]
        self.activation = nn.ReLU() if config.fusion_act == "relu" else nn.Tanh()
...
class BertLMPredictionHead(nn.Module):
    def __init__(self, config, bert_model_embedding_weights):
        ...
        self.decoder = nn.Linear(
            bert_model_embedding_weights.size(1),  #其实不是很懂这个size(1)指的是什么，可以print一下
...
class BertEncoder(nn.Module):
    def __init__(self, config):
        ...
        for n in attn_sublayers:
            num2layers[n] = BertGatedAttention(config, n) # t_attn_output, v_attn_ouput, t_attn_probs, v_attn_probs
    ...
    def forward(self, txt_embedding, img_embedding, txt_attention_mask, img_attention_mask,
        ...
        return all_encoder_layers_t, all_encoder_layers_v, (all_attention_mask_t, all_attention_mask_v) #(all_attention_mask_t, all_attention_mask_v)：【None, None]
...
class BertForVLPreTraining(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        print(self.visual_target_weights)
    ...
    def forward(
        ...
        # Vision loss
        img_loss = 0
        for ix, weight in self.visual_target_weights.items(): #visual_target_weights
